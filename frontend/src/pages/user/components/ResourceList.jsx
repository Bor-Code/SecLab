              ...
              {editingResourceId === resource.id ? (
                <Stack component="form" spacing={1.5} onSubmit={(event) => handleUpdateResource(event, resource.id)}>
                  ...
                  <Stack direction="row" spacing={1} sx={{ justifyContent: 'flex-end' }}>
                    <Button variant="outlined" onClick={handleCancelEditResource} disabled={isSaving}>Cancel</Button>
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ justifyContent: 'space-between' }}>
                  <Stack spacing={0.5}>
                    <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{resource.title}</Typography>
                    <Typography variant="body2" color="primary" component="a" href={resource.url} target="_blank" rel="noopener noreferrer">
                      {resource.url}
                    </Typography>
                    {resource.notes && <Typography variant="body2" color="text.secondary">{resource.notes}</Typography>}
                  </Stack>
                  <Stack direction="row" spacing={1} sx={{ justifyContent: 'flex-end' }}>
                    <Button variant="outlined" size="small" sx={{ minWidth: 72 }} onClick={() => handleStartEditResource(resource)} disabled={isSaving}>Edit</Button>
                    <Button variant="outlined" color="error" size="small" sx={{ minWidth: 72 }} onClick={() => handleDeleteResource(resource.id)} disabled={isSaving}>Delete</Button>
                  </Stack>
                </Stack>
              )}
            </Stack>
